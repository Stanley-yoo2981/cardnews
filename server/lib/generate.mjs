...
톤: 짧은 문장, 한 장에 하나의 생각, 과장 없이 긴장. 느낌표 남발 금지. 클릭베이트 금지.

디자인이 깔끔하게 나오도록 분량을 지켜라(넘치면 디자인이 무너진다):
- 헤드라인(h1, cover_h1, cta_h1): 2~3줄, **한 줄당 12자 이내**, 줄바꿈은 \\n. 전체 28자 이내 권장.
- 강조어(_em): 헤드라인 줄 안에 실제로 들어있는 짧은 단어 하나(2~6자). 없으면 빈 문자열.
- sub: 2~3줄, 한 줄 24자 이내. 문장은 명료하게.
- kicker: 12자 이내 라벨.
- vs_a/b_title: 20자 이내, desc: 18자 이내.
- stats: label 10자 이내 / value 12자 이내. 원문에 결과가 없으면 stats 는 빈 배열 [].
- cards.title: 16자 이내 / desc: 24자 이내.
- checks: 각 22자 이내의 짧은 명령형 구.
...
반드시 아래 JSON 스키마 그대로, 그리고 JSON만 출력하라(설명·코드펜스 금지):
{
  ...
  "cta_h1": "마무리 헤드라인(줄바꿈 \\n)",
  "cta_h1_em": "강조어",
  "caption": "인스타 본문 3~4줄. 상담번호 02-318-2981 포함. 과장 금지.",
  "hashtags": ["#해시태그", "8개 이내"],
